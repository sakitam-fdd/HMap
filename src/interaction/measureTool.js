...
ol.interaction.MeasureTool = function (params) {
   ...
   */
  this.isGeodesic =
    this.options['isGeodesic'] === false ? this.options['isGeodesic'] : true;
  ...
  if (
    this.options['drawStyle'] &&
    typeof this.options['drawStyle'] === 'object'
  ) {
  ...
  if (
    this.options['finshStyle'] &&
    typeof this.options['finshStyle'] === 'object'
  ) {
...
ol.interaction.MeasureTool.prototype.drawEndHandle_ = function (event) {
  ...
  if (this.measureTypes.measureLength['name'] === this.measureType) {
    ...
    this.addMeasureOverlay(
      coordinates,
      this.draw.get('measureResult'),
      'length'
    );
 ...
 * @param event
 */
ol.interaction.MeasureTool.prototype.beforeDrawPointClickHandler = function (
  event
) {
  if (!this.measureHelpTooltip && this.getTool()) {
    ...
    this.measureHelpTooltip instanceof ol.Overlay
  ) {
 ...
 * @param event
 */
ol.interaction.MeasureTool.prototype.afterDrawPointClickHandler = function (
  event
) {
  let helpTooltipElement = this.measureHelpTooltip.getElement();
  if (this.measureTypes.measureLength['name'] === this.measureType) {
    helpTooltipElement.className =
      'hamp-js-measure-move hamp-js-measure-length';
 ...
 * @param type
 */
ol.interaction.MeasureTool.prototype.addMeasureOverlay = function (
  coordinate,
  length,
  type
) {
  ...
  if (type === 'length') {
    ...
    measureResult.innerHTML =
      "总长：<span class='measure-end-label'>" + length + '</span>';
 ...
 * @param coordinate
 */
ol.interaction.MeasureTool.prototype.addMeasureRemoveButton = function (
  coordinate
) {
  let that = this;
  let imageButton = document.createElement('img');
  imageButton.src = this.options['removeButtonSrc']
    ? this.options['removeButtonSrc']
...
ol.interaction.MeasureTool.prototype.removeMeasure_ = function (uuid) {
  ...
  if (overlays && Array.isArray(overlays)) {
    ...
    for (let j = 0, i = 0; j < length; j++) {
      ...
      if (
        overlays[length - i] &&
        overlays[length - i] instanceof ol.Overlay &&
        overlays[length - i].get('uuid') === uuid
      ) {
  ...
  if (overlays && Array.isArray(overlays)) {
    ...
    for (let j = 0, i = 0; j < length; j++) {
      ...
      if (
        overlays[length - i] &&
        overlays[length - i] instanceof ol.Overlay &&
        overlays[length - i].get('layerName') === this.layerName
      ) {
        this.getMap().removeOverlay(overlays[length - i]);
      }
    }
    ...
    if (this.measureTypes.measureLength['name'] === this.measureType) {
      ...
      if (this.isGeodesic) {
        let sourceProj = this.getMap()
          .getView()
          .getProjection();
        let geometry = /** @type {ol.geom.Polygon} */ (geom
          .clone()
        ...
        if (area > 10000000000) {
          output =
            Math.round((area / (1000 * 1000 * 10000)) * 100) / 100 +
            ' ' +
            '万平方公里';
        } else if (area > 1000000 && area < 10000000000) {
          output =
            Math.round((area / (1000 * 1000)) * 100) / 100 + ' ' + '平方公里';
        } else {
      ...
      let sourceProj = this.getMap()
        .getView()
        .getProjection();
      let circle = /** @type {ol.geom.Polygon} */ (geom
        .clone()
        .transform(sourceProj, 'EPSG:4326'));
