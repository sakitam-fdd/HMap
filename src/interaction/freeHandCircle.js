...
ol.interaction.FreeHandCircle.handleMoveEvent_ = function (mapBrowserEvent) {
  if (this.cursor_) {
    ...
    let feature = map.forEachFeatureAtPixel(mapBrowserEvent.pixel, function (
      feature
    ) {
      return feature;
    });
...
ol.interaction.FreeHandCircle.handleDragEvent_ = function (mapBrowserEvent) {
  ...
  this.createCircle(
    this.center_,
    this.mathRadius(this.center_, geometry.getCoordinates())
  );
};
 ...
 * @param radius
 */
ol.interaction.FreeHandCircle.prototype.createCircle = function (
  center,
  radius
) {
  if (!this.getMap()) return;
  ...
  if (!this.circleFeature) {
    ...
    let uuid =
      this.draw && this.draw.get('uuid') ? this.draw.get('uuid') : getuuid();
    ...
    this.circleFeature
      .getGeometry()
      .setCenterAndRadius(params['center'], params['radius']);
  }
};
 ...
 * @private
 */
ol.interaction.FreeHandCircle.prototype.transformCenterAndRadius_ = function (
  center,
  radius
) {
  let center_ = ol.proj.transform(
    center,
    this._getProjectionCode(),
    'EPSG:4326'
  );
  let sourceGeom = new ol.geom.Circle(
    center_,
    this.transformRadius(center_, radius)
  );
 ...
 * @private
 */
ol.interaction.FreeHandCircle.prototype.addLabelFeature_ = function (
  coordinates,
  type
) {
  if (type === 'center') {
 ...
 * @private
 */
ol.interaction.FreeHandCircle.prototype.drawTextLabel_ = function (
  text,
  coordinates
) {
  if (!this.textOverlay) {
 ...
 * @returns {number}
 */
ol.interaction.FreeHandCircle.prototype.transformRadius = function (
  center,
  meterRadius
) {
  try {
 ...
 * @param radius
 */
ol.interaction.FreeHandCircle.prototype.setCenterRadius = function (
  center,
  radius
) {
  if (this.circleFeature && this.circleFeature.getGeometry()) {
